...
1. We are definitely using the Push model in this tutorial. I know this because whenever a product is created or deleted, our Main app actively takes the payload and sends it directly to the subscribers using an HTTP POST request. The subscribers (the Receiver apps) just sit there and wait for the data to be pushed to them without ever having to ask for it.
2. If we switched things up and used the Pull model, the receiver would have to constantly ping the main app to ask, "Hey, are there any new products?" The main advantage of this is that the receiver controls the flow of data; if the main app suddenly has a million updates, the receiver won't get overwhelmed because it only pulls what it can handle. The big disadvantage, though, is the wasted resources. The receiver would waste a ton of network requests asking for updates when absolutely nothing has changed, and there would always be a delay between a product being created and the receiver actually finding out about it.
3. If we decided to skip multi-threading and removed thread::spawn from the notification loop, the program would be forced to notify everyone one by one. This means the app would have to wait for the HTTP request to the first subscriber to completely finish before it could even start sending to the second one. If we had hundreds of subscribers, or if just one of their servers was lagging or offline, it would block the entire execution. The Main app would essentially freeze up and take way too long to finish the original request (like taking 10 seconds just to create a single product).